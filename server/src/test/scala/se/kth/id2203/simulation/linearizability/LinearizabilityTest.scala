...
class LinearizabilityTestBeta extends AnyFlatSpec with Matchers {
  ...
  "Linearizability" should "be implemented" in {
    ...
    val simpleBootScenario = LinearizabilityTestBetaScenario.scenario(clusterSize);
...
object LinearizabilityTestBetaScenario {
  ...
  val startClientOp = Op { (self: Integer) =>
    ...
    StartNode(selfAddr, Init.none[ScenarioClientStart], conf);
  };
